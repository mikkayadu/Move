...
**The model we planned for does not exist.** We had assumed a small E4B variant for latency. Querying the API showed only two Gemma 4 variants served there: `gemma-4-26b-a4b-it` and `gemma-4-31b-it`. We took the 26B mixture-of-experts model, which activates roughly 4B parameters per token and so behaves like a small model at inference time. The repo now ships `npm run models` rather than a hard-coded guess, because a wrong id is a hard 404 on every request.
